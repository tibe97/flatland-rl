...
Transition = namedtuple('Transition', ('timestep', 'state', 'reward', 'nonterminal', 'deadlock'))
...
class ReplayMemory():
	...
	def append(self, state, reward, terminal, deadlock):
		...
		self.transitions.append(Transition(self.t, state, reward, not terminal, deadlock), self.transitions.max)  # Store new transition with maximum priority
	...
	def sample(self, batch_size):
		...
		probs, idxs, tree_idxs, states, actions, returns, next_states, nonterminals, deadlocks = zip(*batch)
		states, next_states, = torch.stack(states), torch.stack(next_states)
		actions, returns, nonterminals, deadlocks = torch.cat(actions), torch.cat(returns), torch.stack(nonterminals), torch.stack(deadlocks)
