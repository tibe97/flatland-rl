...
def main(args):
    ...
    wandb.init(project="Flatland-{}".format(args.wandb_project_name),
               name="{}_{}_agents_on_({}, {})_{}".format(args.run_title,
                                                         args.num_agents,
                                                         args.width,
                                                         args.height,
    ...
    start_print = "About to train {} agents on ({},{}) env.\nParameters:\nmax_num_cities: {}\nmax_rails_between_cities: {}\nmax_rails_in_city: {}\nmalfunction_rate: {}\nmax_duration: {}\nmin_duration: {}\nnum_episodes: {}\nstarting from episode: {}\nmax_steps: {}\neps_initial: {}\neps_decay_rate: {}\nlearning_rate: {}\nlearning_rate_decay: {}\ndone_reward: {}\ndeadlock_reward: {}\n".format(
        args.num_agents, args.width, args.height, max_num_cities_adaptive, args.max_rails_between_cities, args.max_rails_in_city, args.malfunction_rate, args.max_duration, args.min_duration, args.num_episodes, args.start_epoch, max_steps, args.eps, args.eps_decay, args.learning_rate, args.learning_rate_decay, args.done_reward, args.deadlock_reward)
    print(start_print)
    ...
    if args.observation_builder == 'GraphObsForRailEnv':
        observation_builder = GraphObservation(
            args.observation_depth)  # custom observation
        state_size = 12
        rl_agent = Agent(
            args=args,
    ...
    lr_scheduler_policy = StepLR(
        rl_agent.optimizer_action,
        step_size=25,
        gamma=args.learning_rate_decay_policy)
    ...
    for ep in range(
        ...
        for step in range(max_steps):
            ...
            for a in range(env.get_num_agents()):
                ep_controller.save_experience_and_train(
                    a, railenv_action_dict[a], all_rewards[a], next_obs[a], done[a], step, args, ep)
        ...
        if (ep % args.evaluation_interval) == 0:  # Evaluate only at the end of the episodes
            ...
            testing_stats = '\nEpoch ' + str(ep) + ', testing agents on ' + str(args.evaluation_episodes) + ': Avg. done agents: ' + str(avg_done_agents * 100) + '% | Avg. reward: ' + str(
                avg_reward) + ' | Avg. normalized reward: ' + str(avg_norm_reward) + ' | Avg. agents in deadlock: ' + str(avg_deadlock_agents * 100) + '%' + '| LR: ' + str(rl_agent.optimizer_value.param_groups[0]['lr']) + "\n"
            print(testing_stats)
            with open(args.model_path + 'training_stats.txt', 'a') as f:
            ...
            wandb_log_dict.update({"avg_reward": avg_reward,
                                   "done_agents": avg_done_agents,
                                   "deadlock_agents": avg_deadlock_agents})
        ...
        if ep % (args.save_model_interval) == 0:  # backup weights
            ...
            checkpoint_path = args.model_path + "checkpoint_" + \
                str(args.num_agents) + "_agents_on_" + str(args.width) + "_" + str(args.height)
