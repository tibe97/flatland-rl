...
def main(args):
    ...
    stochastic_data = MalfunctionParameters(malfunction_rate=args.malfunction_rate,  # Rate of malfunction occurrence
    ...
    # Construct the environment with the given observation, generators,
    # predictors, and stochastic data
    env = RailEnv(
        width=args.width,
        height=args.height,
    ...
    # Init agent
    dqn = RainbowAgent(args, feature_size)

    # If a model is provided, and evaluate is false, presumably we want to
    # resume, so try to load memory
    if args.model is not None and not args.evaluate:
        ...
        # Init one replay buffer for each agent (TODO) Must be updated when the
        # number of agents change
        mems = [ReplayMemory(args, int(args.memory_capacity / args.num_agents))
                for a in range(args.num_agents)]
        # mem = ReplayMemory(args, args.memory_capacity)  # Init empty replay
        # buffer
    ...
    if args.evaluate:
        ...
        avg_done_agents, avg_reward, avg_norm_reward = test(
            args, 0, 0, dqn, val_mem, metrics, results_dir, evaluate=True)  # Test
        ...
        for ep in trange(1, args.num_episodes + 1):
            ...
            next_state, reward, done, info = env.step(
                railenv_action_dict)  # Env first step
            ...
            for T in range(1, args.T_max + 1):
                ...
                for a in range(env.get_num_agents()):
                    if info['action_required'][a]:
                        ...
                        network_action = dqn.act(state[a])
                ...
                for a in range(env.get_num_agents()):
                    ...
                    * Reward is always in [-1, 1], so we shouldn't need clipping
                    ...
                    if update_values[a]:  # Store transition only if this agent performed action in this time step
                        mems[a].append(
                            state[a],
                            network_action_dict[a],
                            reward[a],
                            done[a])  # Append to own buffer
                ...
                if ep >= args.learn_start:  # Give time to accumulate experiences
                    ...
                    for a in range(args.num_agents):
                        mems[a].priority_weight = min(
                            mems[a].priority_weight + priority_weight_increase, 1)

                    if T % args.replay_frequency == 0:
                        a = np.random.choice(np.arange(args.num_agents))
                        dqn.learn(
                            mems[a])  # Learn randomly from one of the available replay buffer
                        # dqn.learn(mem)  # Train with n-step distributional
                        # double-Q learning
            ...
            if (ep % args.evaluation_interval) == 0:  # Evaluate only at the end of the episodes
                ...
                dqn.eval()  # Set DQN (online network) to evaluation mode
                avg_done_agents, avg_reward, avg_norm_reward = test(
                    args, T, ep, dqn, val_mem, metrics, results_dir)  # Test
                log('T = ' +
                    str(T) +
                    ' / ' +
                    str(args.T_max) +
                ...
                if args.memory is not None:
                    # Save only first replay buffer (?)
                    save_memory(mems[0], args.memory, args.disable_bzip_memory)
            # save_memory(mem, args.memory, args.disable_bzip_memory)

            # Checkpoint the network every 'checkpoint_interval' episodes
            if (args.checkpoint_interval != 0) and (
                    ep % args.checkpoint_interval == 0):
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        default=int(20e3),
        metavar='EPISODES',
        help='Number of episodes before starting training')
    parser.add_argument(
        '--evaluate',
        action='store_true',
        help='Evaluate only')
    ...
    # TODO: Note that DeepMind's evaluation method is running the latest agent
    # for 500K frames ever every 1M steps
    parser.add_argument(
        '--evaluation-size',
        type=int,
        default=500,
    ...
    # Env parameters
    # parser.add_argument('--state_size', type=int, help='Size of state to
    # feed to the neural network') # Depends on prediction_depth
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=20,
        help='Min duration of malfunction')
    parser.add_argument(
        '--max-duration',
        type=int,
        default=50,
        help='Max duration of malfunction')
    parser.add_argument(
        '--observation-builder',
        type=str,
