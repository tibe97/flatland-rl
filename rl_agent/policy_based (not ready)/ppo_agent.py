...
class ActorCriticModel(nn.Module):
    ...
    def __init__(self, state_size, action_size, device, hidsize1=512, hidsize2=256):
        ...
        self.device = device
        self.actor = GAT_actor(state_size, 8, 2, 3, 0.6, 0.2, 4).to(self.device)

        self.critic = GAT_critic(state_size, 8, 1, 3, 0.6, 0.2, 4).to(self.device)

    def forward(self, x):
        raise NotImplementedError

    def _forward_actor(self, x, adj):
        x = F.dropout(x, self.dropout, training=self.training)
        for l in range(self.nlayers):
            if l > 0: 
                residual = x
    ...
    def _forward_critic(self, x, adj):
        x = F.dropout(x, self.dropout, training=self.training)
        for l in range(self.nlayers):
            if l > 0: 
                residual = x
    ...
    def get_actor_dist(self, state):
        action_probs = self._forward_actor(state)
    ...
    def evaluate(self, states, actions):
        ...
        state_value = self._forward_critic(states)
