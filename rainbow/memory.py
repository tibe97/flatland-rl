...
# Value depends on state_size, now is args.prediction_depth * 4 + 4 TODO
...
# Segment tree data structure where parent node values are sum/max of
...
class SegmentTree():
    ...
    # Searches for a value in sum tree and returns value, data index and tree
    # index
    def find(self, value):
...
class ReplayMemory():
    def __init__(self, args, capacity):
        ...
        # Initial importance sampling weight β, annealed to 1 over course of
        # training
        self.priority_weight = args.priority_weight
        # Prioritised experience replay exponent (α)
    ...
    def append(self, state, reward, terminal, deadlock):
        state = torch.from_numpy(state)
        # state = state[-1].mul(255).to(dtype=torch.uint8,
        # device=torch.device('cpu'))  # Only store last frame and discretise
        # to save memory
        # Discretize to save memory
    ...
    def _get_sample_from_segment(self, segment, i):
        ...
        while not valid:
            ...
            if (self.transitions.index - idx) % self.capacity > self.n and (idx - \
                self.transitions.index) % self.capacity >= self.history and prob != 0:
        ...
        next_state = torch.stack([trans.state for trans in transition[self.n:self.n +
                                                                      self.history]]).to(device=self.device).to(dtype=torch.float32).div_(255)
    ...
    def sample(self, batch_size):
        # Retrieve sum of all priorities (used to create a normalised
        ...
        probs, idxs, tree_idxs, states, actions, returns, next_states, nonterminals, deadlocks = zip(
            *batch)
        states, next_states, = torch.stack(states), torch.stack(next_states)
        actions, returns, nonterminals, deadlocks = torch.cat(actions), torch.cat(
            returns), torch.stack(nonterminals), torch.stack(deadlocks)
        ...
        return tree_idxs, states, actions, returns, next_states, nonterminals, deadlocks, weights
