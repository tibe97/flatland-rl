...
class Agent:
    ...
    def __init__(self, args, state_size, obs_builder):
        ...
        self.qnetwork_value_local = GAT_value(state_size, 8, 1, args.gat_layers, 0.6, 0.2, 4, args.flow).to(device)
        self.qnetwork_value_target = copy.deepcopy(self.qnetwork_value_local)
        #self.qnetwork_action = DQN_action(state_size).to(device)
        self.qnetwork_action = GAT_action(state_size, 8, 2, args.gat_layers, 0.6, 0.2, 4, args.flow).to(device)
