...
class Agent:
    ...
    def __init__(self, args, state_size, obs_builder):
        ...
        self.qnetwork_value_local = GAT_value(
            state_size,
            10,
            1,
            args.gat_layers,
        ...
        self.qnetwork_action = GAT_action(
            state_size,
            10,
            2,
            args.gat_layers,
        ...
        self.optimizer_value = optim.Adam(
            self.qnetwork_value_local.parameters(),
            lr=self.learning_rate)
        self.optimizer_action = optim.Adam(
            self.qnetwork_action.parameters(),
            lr=self.learning_rate)
    ...
    def save(self, filename):
        ...
        torch.save(
            self.qnetwork_value_local.state_dict(),
            filename + "_value_local.pth")
        torch.save(
            self.qnetwork_value_target.state_dict(),
    ...
    def act(self, state, eps=0.0, eval=True):
        ...
        if eval:
            self.qnetwork_value_local.eval()
            with torch.no_grad():
                out_value = self.qnetwork_value_local(
        ...
        for i, res in enumerate(
                zip(out_value, out_action)):  # RES = (node_value, (value_stop, value_go))
        ...
        for handle in out_mapped.keys():
            ...
            if self.evaluation_mode:  # test time
                ...
                agents_best_path_values.update(
                    {handle: [best_path[0], action.item(), log_prob, best_path[1][0], m.probs[action]]})
            else:
                if random.random() >= eps:
                    ...
                    log_prob = m.log_prob(action)
                    agents_best_path_values.update(
                        {handle: [random_path[0], action.item(), log_prob, random_path[1][0], m.probs[action]]})
    ...
    def _append_sample(self, state, reward, next_state, done, deadlock):
        ...
        if not deadlock and not done:
            # 0 is just a placeholder value for the agent (we don't care which
            # one here)
            ...
            # selected node value, which is the path with max value
            S_value = path_values[0][3]
            ...
            # append random value,it won't be considered
    ...
    def learn(self, gamma, ep):
        ...
        for i, state in enumerate(next_states):
            if not deadlocks[i] and not dones[i]:
                # 0 is just a placeholder value for the agent (we don't care
                # which one here)
                preprocessed_next_state = self.obs_builder.preprocess_agent_obs(
                    state, 0)
                # Choose path to take at the current switch
                path_values = self.act(
                    preprocessed_next_state, eps=0, eval=False)
                # selected node value, which is the path with max value
                S_value = path_values[0][3]
                Q_targets_next.append(S_value)
            else:  # if agent is done or is in deadlock, we don't need to compute any value. Value is reward at the end
                # append random value,it won't be considered
                Q_targets_next.append(torch.tensor([rewards[i]]))
        ...
        deadlocks = torch.tensor(
            [1 if deadlock else 0 for deadlock in deadlocks])
        ...
        self.soft_update(
            self.qnetwork_value_local,
            self.qnetwork_value_target,
            TAU)
...
class ReplayBuffer:
    ...
    def __init__(self, network_type, buffer_size, batch_size):
        ...
        self.memory_deadlocks = deque(
            maxlen=buffer_size)  # save deadlock experiences
    ...
    def sample(self):
        ...
        if len(
            self.memory_dones) > self.batch_size // 10 * 2 and len(
            self.memory_deadlocks) > self.batch_size // 10 * 3 and len(
                self.memory_other) > self.batch_size // 10 * 5:
